...
for lat_start = lat_starts
    for lon_start = lon_starts
        filename = fullfile(subfolderPath, sprintf('20070717063000_%s_%s_%s_limit.mat', num2str(lat_start), num2str(lon_start), num2str(alt_start)));
        if exist(filename,'file')
            disp([filename,'已存在，跳过循环。']);
            continue
        else
            [lat, lon, alt] = igrfLimitLine(time, lat_start, lon_start, alt_start, 'geod', distance, nsteps);
            limit_alt = alt>-1;
