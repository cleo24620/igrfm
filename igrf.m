...
% time rather than the time itself（提供"COEFS"比"TIME"更有效率）. This mode is useful when making
% multiple calls to the function while keeping the time the same (meaning
% the coefficients will be the same for each run（同一时间，则COEF不变）) as loading the
% coefficients can be the most time-consuming part of the function（加载系数是函数最费时的部分）. The
% coefficient vector can be easily loaded using the function LOADIGRFCOEFS（可使用"LOADIGRFCOEFS"加载系数）.
% This mode is assumed（采用此model） when all the coordinate inputs are scalars and the
...
% BY, and BZ as（IGRF给出的其它参数可由BX, BY, and BZ算出）:
% 
%   Horizonal intensity: hypot(BX, BY) (i.e., sqrt(BX.^2 + BY.^2) )
%   Total intensity: hypot(BX, hypot(BY, BZ))
%   Declination（磁偏角）: atan2(BY, BX)
%   Inclination（轨道倾角）: atan(BZ./hypot(BX, BY))
% 
% This function relies on having the file igrfcoefs.mat in the MATLAB
% path to function properly when a time is input（该函数依赖于 MATLAB 路径中的 igrfcoefs.mat 文件才能在输入时间时正常运行）. If this file cannot be
% found, this function will try to create it by calling GETIGRFCOEFS.
% 
% The IGRF is a spherical harmonic expansion（球谐展开式） of the Earth's internal
% magnetic field. Currently, the IGRF model is valid between the years 1900
% and 2015（有效时间范围）. See the health warning for the IGRF model here:
...
%   surface（--> "geod", not "ecef"）. For geocentric coordiates, the radius in km from the center of
%   the Earth.
%   -COORD: String specifying the coordinate system to use. Either
%   'geocentric' or 'geodetic' (optional, default is geodetic). Note that
%   only geodetic（"geod", not "ecef"） coordinates have been verified（只有 "geodetic" 被验证）.
% 
% Outputs:
...
function [Bx, By, Bz] = igrfv(time, latitude, longitude, altitude, coord)
    ...
    Rearth_km = 6371.2;  % 地球直径

    %%% CHECK INPUT VALIDITY %%%
    % Convert time to a datenumber if it is a string. 处理时间
    if ischar(time)
        time = datenum(time);
    end

    % Make sure time has only one element.  确保时间只有1个元素
    if numel(time) > 1
    ...
    costheta = cos((90 - latitude(:))*pi/180);  % theta = 90 - latitude(:) 单位：弧度
    ...
    if nargin < 5 || isempty(coord) || strcmpi(coord, 'geodetic') || strcmpi(coord, 'geod') || strcmpi(coord, 'gd')
        a = 6378.137; f = 1/298.257223563; b = a*(1 - f);
        rho = hypot(a*sintheta, b*costheta);
        r = sqrt( altitude(:).^2 + 2*altitude(:).*rho + (a^4*sintheta.^2 + b^4*costheta.^2) ./ rho.^2 );
        cd = (altitude(:) + rho) ./ r;
    ...
    elseif strcmpi(coord, 'geocentric') || strcmpi(coord, 'geoc') || strcmpi(coord, 'gc')
        ...
        error('igrf:coordInputInvalid', ['Unrecognized command ' coord ' for COORD input.']);
    end
    ...
    for n = 1 : nmax
        ...
        %     dP(n-2, m)
        dP = [bsxfun(@minus, bsxfun(@times, (2*n - 1)./sqrt(n^2 - m(1:end-1).^2), bsxfun(@times, costheta, lastdP_1) - bsxfun(@times, sintheta, lastP)), bsxfun(@times, sqrt(((n - 1)^2 - m(1:end-1).^2)./(n^2 - m(1:end-1).^2)), lastdP_2)), zeros(size(costheta))];
        if n > 1
            dP(:, end) = sqrt(1 - 1/(2*n))*(sintheta.*lastdP_1(:, end) + costheta.*lastP(:, end));
        ...
        if anysinthetanot0
            Bp(~sintheta0) = Bp(~sintheta0) - 1./sintheta(~sintheta0).* a_r(~sintheta0).*sum(bsxfun(@times, m, (-gsin(~sintheta0, :) + hcos(~sintheta0, :)).*P(~sintheta0, :)), 2);
        end
        if anysintheta0
            Bp(sintheta0) = Bp(sintheta0) - costheta(sintheta0).* a_r(sintheta0).*sum((-gsin(sintheta0, :) + hcos(sintheta0, :)).*dP(sintheta0, :), 2);
        end
...
function [Bx, By, Bz] = igrfs(time, latitude, longitude, altitude, coord)
    ...
    for Pindex = 2:Pmax
        ...
        if m < n && Pindex ~= 3 % (Pindex=3 is n=1, m=1, initial cond. above)
            last1n = Pindex - n;
            last2n = Pindex - 2*n + 1;
            P(Pindex) = (2*n - 1)/sqrt(n^2 - m^2)*costheta*P(last1n) - sqrt(((n-1)^2 - m^2) / (n^2 - m^2)) * P(last2n);
            dP(Pindex) = (2*n - 1)/sqrt(n^2 - m^2)*(costheta*dP(last1n) - sintheta*P(last1n)) - sqrt(((n-1)^2 - m^2) / (n^2 - m^2)) * dP(last2n);
        elseif Pindex ~= 3
            lastn = Pindex - n - 1;
            P(Pindex) = sqrt(1 - 1/(2*m))*sintheta*P(lastn);
            dP(Pindex) = sqrt(1 - 1/(2*m))*(sintheta*dP(lastn) + costheta*P(lastn));
        end
        ...
        if m == 0 % Implies h = 0, so only coefficient in gh is g
            ...
            if sintheta == 0 % Use different formula when dividing by 0.
                Bp = Bp - costheta*a_r*(-gh(coefindex)*sinphi(m) + gh(coefindex+1)*cosphi(m))*dP(Pindex);
            else
                Bp = Bp - 1/sintheta*a_r*m*(-gh(coefindex)*sinphi(m) + gh(coefindex+1)*cosphi(m))*P(Pindex);
            end
