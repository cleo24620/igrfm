...
% ECEF2GEOD Convert ECEF ((Earth-centered Earth-fixed) 以地球球心为笛卡尔坐标系原点，以 (x,y,z) 表示点的位置) coordinates to geodetic coordinates.
% 
...
% (WGS84) ellipsoid(椭球) model of the Earth is assumed.
% 
% Inputs:
%   -X: x coordinates of the point in meters.
%   -Y: y coordinates of the point in meters.
%   -Z: z coordinates of the point in meters.
%   -TOL (容许误差): Maximum error tolerance in the latitude in radians (optional,
%   default is 1e-12).
...
% See also: ECEF2LLA, GEOD2ECEF.
    ...
    rd = hypot(x, y);
    [latitude, Nphi] = recur(asin(z ./ hypot(x, hypot(y, z))), z, a, e2, rd, tol, 1);
